...
**Why it matters:** Comparing callbacks, promises, `async`/`await`, and `fetch`
side by side shows why the language kept evolving, and when each style is the
right tool.

**Source:** [`src/modules/`](src/modules/)

**Run:** serve statically; click the button when prompted and watch the ordering in the panel on the page (also mirrored to the console).

[**▶ Live demo**](https://stackblitz.com/github/OlgaShtante/js-practice/tree/main/4-promises-and-async/2-async-await-fetch?view=preview)
