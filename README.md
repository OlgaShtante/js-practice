...
their README that opens the task in StackBlitz straight from GitHub — no local
setup. It's a real demo, so give it a few seconds on first load while StackBlitz
boots the sandbox (running it locally with `serve` warms up the same way).
...
the demo in StackBlitz from GitHub and opens the rendered page — give it a few
seconds to spin up. A [task index](public/index.html) gathers the demos too.
